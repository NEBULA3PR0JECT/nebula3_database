...
def normalize_movie(mid):
    action_data = mdb.get_all_expert_data("Actions", mid)
    object_data = mdb.get_all_expert_data("Object", mid)
    clip_data = mdb.get_clip_data(mid)
    vcomet_data = mdb.get_vcomet_data(mid)
    movie_info = mdb.get_movie_info(mid)
    frame_array = []
